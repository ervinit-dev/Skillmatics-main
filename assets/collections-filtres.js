...
(function(e) {
  ...
  var obj = {
    ...
    showMoreHandler: function() {
      ...
      e.ajax({
        ...
        success: function(data) {
          ...
          if (jQuery('.areviews_product_item').length > 0 && typeof show_infiniti_areviews === 'function'){show_infiniti_areviews();}data = null;
        },
      ...
      if (e(".filtres-js").length > 0) {
        e('.filtres-js:not(.clear-filters) a').unbind().click(function(event) {
          ...
          obj.ajaxClick(obj.getAjaxLink(queryParams));
                ...
                //collection page age dynamic filter

      if (e(".collection-age-btns").length > 0) {
        e('.collection-age-btns:not(.clear-filters) a').unbind().click(function(event) {
          event.preventDefault();
          delete queryParams.page;
    ...
    initFiltresRemoveEvent: function() {
      if (e(".filtres-remove-js").length > 0) {
        e('.filtres-remove-js a').unbind().click(function(event) {
          ...
          obj.ajaxClick(obj.getAjaxLink(queryParams));
           //collection page age dynamic filter
           $('.collection-age-btns:not(.clear-filters) a').removeClass('active');
           //collection page age dynamic filter
        });
      };
    },
    ...
    getCollectionContent: function(url) {
      ...
      var params = {
        ...
        success: function(data) {
          ...
          obj.initPaginationEvent();
          obj.initSidebar();
          if (jQuery('.areviews_product_item').length > 0 && typeof show_infiniti_areviews === 'function'){show_infiniti_areviews();}obj.initShowMore();
        },
        error: function(XMLHttpRequest, textStatus) {
          obj.hidePreloader();
          alert("error")
    ...
    pageData: function(data) {
      ...
      e.type(content.html()) === "undefined" ? $(".product-pagintion").empty() : e(".product-pagintion").replaceWith(content);
